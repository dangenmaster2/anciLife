    ...
    return (
        <Pressable style={styles.gridItem} onPress={onPress}>
            <View style={styles.imageContainer}>
                <Animated.Image source={item.img} style={[styles.image, animatedStyle]}/>
                {isSelected && (<AntDesign 
...
const UserInterests = ({userTopicInterests, setUserTopicInterests}) => {
    ...
    const gridPressHandler = (item) => {
        ...
        if (!selectedId.includes(item.id)) {
            ...
            newUserTopicInterests = [...userTopicInterests, {item_topic:item.topic, item_id:item.id}]
            ...
            newUserTopicInterests = newUserTopicInterests.filter(interests => interests.item_id === item.id);
            setSelectedId(newSelectedId);
            setUserTopicInterests(newUserTopicInterests);
        }
    }
